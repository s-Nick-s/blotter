...
#' @rdname initPosPL
.initPosPL <- function(initDate="1950-01-01", ..., initPosQty=0, initClientId=0, initCcyMult=1) #TODO add other init values to function as well for cost basis
{ # @author Peter Carl
    posPL <- xts( as.matrix(t(c(initPosQty,initClientId,initCcyMult,0,0,0,0,0,0,0,0))), order.by=as.POSIXct(initDate, ...=...), ...=... )
    colnames(posPL) <- c('Pos.Qty', 'Client.Id', 'Ccy.Mult', 'Pos.Value', 'Pos.Avg.Cost', 'Txn.Value',  'Period.Realized.PL', 'Period.Unrealized.PL','Gross.Trading.PL', 'Txn.Fees', 'Net.Trading.PL')
