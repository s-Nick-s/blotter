...
{ # @author Peter Carl, Brian Peterson
  ...
  if(nrow(Prices)<1) {
	...
	if(nrow(priorPL)==0) {
		...
		if(inherits(startDate,"POSIXlt")) order.by.tmp$sec <- order.by.tmp$sec - 1
		else order.by.tmp <- order.by.tmp - 1;
		priorPL = xts(t(rep(0,ncol(priorPL))),order.by=c(order.by.tmp))  # MUST CHANGE - POSIXlt doesnt take arithmetic operators
		colnames(priorPL)<-cn
	}
	...
	# IF NO NEW PRICES AND NO NEW TXNS - NO NEED TO DO ANYTHING!
	if( nrow(Prices)>0 || nrow(Txns)>0 )
	{
